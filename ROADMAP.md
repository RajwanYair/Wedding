...
_The fastest, most accessible, RTL-native, offline-first, open-source wedding manager on the web.
Self-hostable in one click. Operable on flaky 3G in Hebrew. Integrated end-to-end with WhatsApp.
Planner-grade analytics. AI-optional, privacy-first. $0/month self-hosted. Best-in-class DX._

### State (2026-06-02, v32.2.0)
...
3. **DX configuration bloat removed** — LiveServer, webhint, and all IE-targeting noise eliminated;
   extensions.json cleaned; DX fully production-clean.
...
| Tests           | Vitest 4 + Playwright 1.60 + Stryker        | Latest      | Active   |
| Lint            | ESLint 10 + Stylelint 17 + HTMLHint         | Latest      | Active   |
| CI/CD           | GitHub Actions (23 workflows + OIDC)        | Latest      | Active   |
...
| 37  | Copilot agents (9)  | Active   | **KEEP + enhance**                 | Full coverage            |
...
### 3.11 VS Code & GitHub Integration

| #   | Decision                 | Current | Verdict                           | Rationale           |
| --- | ------------------------ | ------- | --------------------------------- | ------------------- |
| 53  | 9 Copilot agents         | Active  | **KEEP + enhance prompts**        | Domain-specific     |
| 54  | 17 prompts               | Active  | **EVOLVE → add workflow prompts** | Cover all workflows |
| 55  | 9 skills                 | Active  | **EVOLVE → add supabase skill**   | Missing coverage    |
| 56  | 23 GHA workflows         | Active  | **KEEP + OIDC**                   | Full CI/CD          |
| 57  | MCP servers (7)          | Active  | **EVOLVE → add context7**         | Up-to-date docs     |
| 58  | Parent MyScripts tooling | Partial | **EVOLVE → sync learnings**       | Shared configs      |
| 59  | VS Code extensions (46)  | Active  | **AUDIT → prune unused**          | DX signal-to-noise  |
...
| OSS         | ❌         | ❌          | ❌        | ❌         | **MIT**               | **Lead**       |
| DX (agents) | None       | None        | None      | None       | **9 agents+MCP**      | **Lead**       |
| CI gates    | Basic      | Basic       | Basic     | Basic      | **23 workflows**      | **Lead**       |

### 4.3 DX & Tooling Comparison

| Dimension          | Cal.com | Supabase OSS | Linear | Vercel | **Ours**       | Verdict  |
| ------------------ | ------- | ------------ | ------ | ------ | -------------- | -------- |
| Copilot agents     | 0       | 0            | 0      | 0      | **9**          | **Lead** |
| Instruction files  | 0       | 1            | 0      | 1      | **8**          | **Lead** |
| Prompt files       | 0       | 0            | 0      | 0      | **17**         | **Lead** |
| Skill files        | 0       | 0            | 0      | 0      | **9**          | **Lead** |
| MCP servers        | 0       | 1            | 0      | 0      | **7**          | **Lead** |
| GHA workflows      | 5-8     | 10-12        | 3-5    | 4-6    | **23**         | **Lead** |
| Mutation testing   | ❌      | ❌           | ❌     | ❌     | **✅ Stryker** | **Lead** |
| Canonical facts CI | ❌      | ❌           | ❌     | ❌     | **✅**         | **Lead** |

### 4.4 Key Insight

Every competitor uses TypeScript. We lead in bundle, DX, and offline — but TS is table stakes.
Adopt TS — the migration cost at 0 TSC errors is minimal. Our DX tooling (agents, prompts,
skills, MCP) is already best-in-class among OSS projects — defend and extend.
...
| **Supabase OSS**  | MCP server pattern                      | Our MCP config      |
| **Vercel**        | Edge-first architecture                 | CF Worker pattern   |
| **Turborepo**     | Monorepo task graph                     | MyScripts structure |
...
11. **9 Copilot agents** — domain-specific AI assistance
12. **MCP integration** — GitHub + Supabase + Playwright in chat
13. **23 GHA workflows** — automated everything
