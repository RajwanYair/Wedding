// =============================================================================
// Wedding Manager — Repo Sanity Suite v32.0.0
...
describe("Version alignment", function () {
  it("package.json is v32.0.0", function () {
    assert.equal(packageJson.version, "32.0.0");
  });

  it("src/core/config.js exports APP_VERSION v32.0.0", function () {
    assert.ok(read("src/core/config.js").includes('APP_VERSION = "32.0.0"'));
  });

  it("public/sw.js uses wedding-v32.0.0 cache", function () {
    assert.ok(serviceWorker.includes("wedding-v32.0.0"));
  });

  it("README version badge references v32.0.0", function () {
    assert.ok(readme.includes("version-v32.0.0"));
  });

  it("Copilot instructions title references v32.0.0", function () {
    assert.ok(
  ...
  it("ARCHITECTURE.md header references v32.0.0", function () {
    assert.ok(architecture.includes("# Wedding Manager — Architecture (v32.0.0)"));
  });

  it("src/types.d.ts header references v32.0.0", function () {
    assert.ok(
      typesSource.includes(
        "src/types.d.ts — Shared type definitions for the Wedding Manager (v32.0.0)",
      ),
    );
  });
});
