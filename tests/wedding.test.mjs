// =============================================================================
// Wedding Manager — Repo Sanity Suite v26.0.0
...
describe("Version alignment", function () {
  it("package.json is v26.0.0", function () {
    assert.equal(packageJson.version, "26.0.0");
  });

  it("src/core/config.js exports APP_VERSION v26.0.0", function () {
    assert.ok(read("src/core/config.js").includes('APP_VERSION = "26.0.0"'));
  });

  it("public/sw.js uses wedding-v26.0.0 cache", function () {
    assert.ok(serviceWorker.includes("wedding-v26.0.0"));
  });

  it("README version badge references v26.0.0", function () {
    assert.ok(readme.includes("version-v26.0.0"));
  });

  it("Copilot instructions title references v26.0.0", function () {
    assert.ok(
      copilotInstructions.includes(
        "# GitHub Copilot Instructions — Wedding Manager v26.0.0",
      ),
    );
  });

  it("ARCHITECTURE.md header references v26.0.0", function () {
    assert.ok(architecture.includes("# Wedding Manager — Architecture (v26.0.0)"));
  });

  it("src/types.d.ts header references v26.0.0", function () {
    assert.ok(
      typesSource.includes(
        "src/types.d.ts — Shared type definitions for the Wedding Manager (v26.0.0)",
      ),
    );
  });
});
