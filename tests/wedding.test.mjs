// =============================================================================
// Wedding Manager — Repo Sanity Suite v12.5.5
...
describe("Version alignment", function () {
  it("package.json is v12.5.5", function () {
    assert.equal(packageJson.version, "12.5.5");
  });

  it("src/core/config.js exports APP_VERSION v12.5.5", function () {
    assert.ok(read("src/core/config.js").includes('APP_VERSION = "12.5.5"'));
  });

  it("public/sw.js uses wedding-v12.5.5 cache", function () {
    assert.ok(serviceWorker.includes("wedding-v12.5.5"));
  });

  it("README version badge references v12.5.5", function () {
    assert.ok(readme.includes("version-v12.5.5"));
  });

  it("Copilot instructions title references v12.5.5", function () {
    assert.ok(
      copilotInstructions.includes(
        "# GitHub Copilot Instructions — Wedding Manager v12.5.5",
      ),
    );
  });

  it("ARCHITECTURE.md header references v12.5.5", function () {
    assert.ok(architecture.includes("# Wedding Manager — Architecture (v12.5.5)"));
  });

  it("src/types.d.ts header references v12.5.5", function () {
    assert.ok(
      typesSource.includes(
        "src/types.d.ts — Shared type definitions for the Wedding Manager (v12.5.5)",
      ),
    );
  });
});
