// =============================================================================
// Wedding Manager — Repo Sanity Suite v17.0.0
...
describe("Version alignment", function () {
  it("package.json is v17.0.0", function () {
    assert.equal(packageJson.version, "17.0.0");
  });

  it("src/core/config.js exports APP_VERSION v17.0.0", function () {
    assert.ok(read("src/core/config.js").includes('APP_VERSION = "17.0.0"'));
  });

  it("public/sw.js uses wedding-v17.0.0 cache", function () {
    assert.ok(serviceWorker.includes("wedding-v17.0.0"));
  });

  it("README version badge references v17.0.0", function () {
    assert.ok(readme.includes("version-v17.0.0"));
  });

  it("Copilot instructions title references v17.0.0", function () {
    assert.ok(
      copilotInstructions.includes(
        "# GitHub Copilot Instructions — Wedding Manager v17.0.0",
      ),
    );
  });

  it("ARCHITECTURE.md header references v17.0.0", function () {
    assert.ok(architecture.includes("# Wedding Manager — Architecture (v17.0.0)"));
  });

  it("src/types.d.ts header references v17.0.0", function () {
    assert.ok(
