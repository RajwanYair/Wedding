# GitHub Copilot Instructions — Wedding Manager v32.1.0
...
| Tests | `npm test` — **6619 tests** across 467 files · 0 Node warnings · 190 utils |
...
src/                 # Active ESM source (Vite entry: src/main.js)
  ...
  services/          # auth · sheets · sheets-impl · backend · presence · supabase · ai-proxy
  utils/             # phone · date · sanitize · misc · vendor-negotiate · vendor-sla
...
.github/             # workflows · instructions · copilot · agents · prompts · skills · Dependabot
.vscode/             # settings · extensions · tasks · mcp.json
```
...
6. `.github/instructions/tests.instructions.md` — test count + file count
7. `.github/workflows/ci.yml` — version in header comment
8. `ARCHITECTURE.md` — version in h1; `ROADMAP.md` — current state block + release table
9. `AGENTS.md` — canonical facts block (version + tests + utils)
10. `npm run lint && npm test` — 0 fail, 0 warn, 0 Node warnings
11. `git tag vX.Y.Z && git push --tags`
...
callAiProxy(prompt, opts?)          // AI inference via CF Worker (src/services/ai-proxy.js)
refreshAiSuggestions()              // re-run AI dashboard suggestions widget
applyFloorPlanPreset(preset)        // set tables to preset layout ("banquet"|"classroom"|"cocktail")
exportFloorPlanSvg()                // download current floor plan as SVG
toggleSeatingConstraints()         // show/hide seating constraint config panel
startVendorNegotiation()           // open vendor negotiation panel
submitVendorOffer()                 // submit negotiation counter-offer
generatePaymentSchedule()          // generate installment payment schedule for vendor
```
