...
> Wire `audit:utils` into CI, ICU `--strict` plural arms for all 6 locales
> (76 violations fixed across HE/AR/ES/FR/RU), vendor-contract lifecycle,
> plugin-review pipeline, website-preview builder, Stripe receipt generator,
> RSVP dietary-cascade questions, floor-plan presets (banquet/U-shape/cocktail),
> workspace invitation helpers (create/accept/revoke/stats).
> Test count: 5663 across 414 files; 0 lint warnings, 0 Node warnings.

### Added

- `src/utils/vendor-contracts.js` — contract status FSM, validation, expiry helpers (S620)
- `src/utils/plugin-review.js` — review submission/approval pipeline (S621)
- `src/utils/website-preview.js` — preview builder, countdown, URL generation (S622)
- `src/utils/payment-receipt.js` — receipt number, subtotal, tax, formatting (S623)
- `src/utils/dietary-cascade.js` — meal-triggered dietary follow-up questions (S624)
- `src/utils/floor-plan-presets.js` — banquet, U-shape, cocktail layout templates (S625)
- `src/utils/workspace-invite.js` — invite create/accept/revoke/stats lifecycle (S626)
- CI gate: `audit:utils` step in `ci.yml` (S618)
- CI gate: ICU `--strict` plural validation for all locales (S619)

### Fixed

- 76 ICU plural violations across AR (zero/two/few/many), HE (two/many), ES/FR/RU (many) (S619)
...
- **S564 AI edge proxy scaffold** — `worker/index.js`, `worker/router.js`,
  `worker/wrangler.toml`; `/health` + `/ai/chat`; CORS allowlist.
- **S565 Provider adapters** — OpenAI, Anthropic, Gemini, Ollama in
  `worker/providers.js` with normalised request/response shape.
- **S566 Streaming + BYO key** — `streamAi()` async generator;
  `proxyUrl` setting in `wedding_v1_ai_settings`; SSE branch in worker.
- **S567 Cmd-K command palette** — promotes search index to a true
  palette: Sync now / Add guest / Open settings / Export CSV /
  Toggle theme; +5 i18n keys × 6 locales.
- **S568 TypeScript pilot** — ADR-045; `tsconfig.worker.json`
  (strict noEmit); first `.ts` file (`worker/types.ts`).
- **S569 Container queries** — vendors (stacked), expenses (column),
  gallery (denser), gifts (stacked), invitation (compact).
- **S570 light-dark() + color-mix(in oklch)** — neutral surface / text /
  border / bg tokens; accent hover/active/on-surface variants.
- **S571 Animation Timeline API** — `@supports (animation-timeline:
  view())` reveals for `.stat-card` and `.analytics-kpi-grid` children;
  reduced-motion safe.
- **S572 `<wedding-badge>` Web Component** — Shadow DOM, 5 variants,
